...
internal record Area(string Name, string Prefix)
{
    private Area? parent = null;

    public string Comment
    {
        get => field ?? $"Provides access to constants under {Path}";
        set;
    } = null;
    ...
    public static Area Load(List<Constant> constants, string rootArea = "Constants", string comment = "Provides access project-defined constants.")
    {
        ...
        {
            //  Splits: ([area].)*[name]
            var parts = constant.Name.Split(["."], StringSplitOptions.RemoveEmptyEntries)
    ...
    private static void SortArea(Area area)
    {
    ...
    private static Area GetArea(Area area, IEnumerable<string> areaPath)
    {
