    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(
            ...
            (spc, args) =>
            {
                ...
                var strings = EmbeddedResource.GetContent($"ResourceManager.sbntxt");
                var template = Template.Parse(strings);
                var output = template.Render(new { Namespace = ns }, member => member.Name);
                spc.AddSource("MyAssembly.Strings.g.cs", SourceText.From(output, Encoding.UTF8));
            });

        var files = context.AdditionalTextsProvider
            .Combine(context.AnalyzerConfigOptionsProvider)
            .Where(x =>
                x.Right.GetOptions(x.Left).TryGetValue("build_metadata.ResxCode.MyAssemblyStrings", out var resourceString)
